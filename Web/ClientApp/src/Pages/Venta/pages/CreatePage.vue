...
const {
  ...
  [
      {cantidad:2, categoria:'ss',descripcion:'ssss', id:1013, nombre: 'ssd', precioMayoreo: 3,
...
const {submitCustomFormWithData} = useAtlasComposableCustomFormActions('venta', true, )
...
const sendForm = () =>{
  const purchaseDetails = getDetails()
  const prods = expandBasedOnValue( mapToKeyValue(purchaseDetails.productos,'cantidad') )

  const newObj = {
    pago: purchaseDetails.pago,
    metodoPago: purchaseDetails.pago,
    cambio: purchaseDetails.cambio,
    total: purchaseDetails.total,
    productos: prods
  }
  
  submitCustomFormWithData(newObj)
}


const {mapToKeyValue, expandBasedOnValue} = useAtlasComposableMapKeyValue()
        ...
        >
            ...
            <div class="flex flex-col">
              ...
              <div class="flex flex-col">
                <div class="grid grid-cols-3 gap-2 m-2">
                  <button class="btn btn-info w-full" @click="scanning = true">
                    Escanaer
                  </button>
              ...
              <div class="h-90 md:h-96 overflow-y-scroll pt-1 pb-1 h-sceen">
                <ul class="list bg-base-100 mt-1 m-2">
                  <div class="grid grid-cols-2">
                    ...
                    <template v-else> no items scanned </template>
                  </div>
                </ul>
              </div>
            </div>
          ...
          <swiper-slide class="w-full">
            <div class="flex flex-col h-[calc(84.3vh)] md:h-[calc(85.4vh)]">
              ...
              <div class="grid grid-cols-2 mx-2 mb-2 gap-2">
                <buton @click="swipePrev" class="btn btn-md w-full"
                  >Anterior</buton
                >
                <buton @click="swipeNext" class="btn btn-md w-full"
                  >Siguiente</buton
                >
              </div>
          ...
          <swiper-slide class="w-full">
            <div class="flex flex-col h-[calc(84.3vh)] md:h-[calc(85.4vh)]">
              ...
              <div class="flex-1 flex-wrap text-center h-full">
                <div class="grid grid-cols-1 gap-2 px-2 py-3  bg-gray-400  text-xl   ">
                  <div>
                    Metodo de Pago
                  </div>
                  <div class="">
                    <select class="select select-info" v-model="metodoPago">
                      <option value="efectivo" selected>Efectivo</option>
                      <option disabled>Tarjeta</option>
                      <option disabled>Transferencia</option>
                      <option disabled>SPEI</option>
                    </select>
                  </div>
                  <div class="">
                    <button class="btn btn-info w-full ">Capturar</button>
                  </div>
                </div>

                <div class="h-125 overflow-y-scroll">
                  <ul class="list bg-base-100 mt-1 m-2 ">
                    <div class="grid grid-cols-2 py-2 gap-2  md:place-items-center text-xl md:text-4xl">
                      ...
                      <div class="border-b-2 border-solid ">
                        <input
                        @input="setPago( +($event.target as HTMLInputElement).value)"
                        type="number" class="input w-full validator text-center md:input-ghost  text-xl md:text-4xl " required 
                        placeholder="Pago"
                        :min="total"
                        />
                        <p class="hidden sm:visible validator-hint text-center mb-2">Pago mayor/igual {{ total }}</p>
                      </div>
                    ...
                    <button class="btn btn-success" :disabled="!isValidPayment" @click="sendForm" >Applicar Pago</button>
                  </ul>
                </div>
              </div>
            </div>
